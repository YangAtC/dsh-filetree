...
async function handleRead(path) {
  ...
  let info
  try { info = await stat(target) } catch { return { error: 'not-found' } }
  ...
  if (IMAGE_EXTS.has(ext)) {
    ...
    if (bytes.length > MAX_IMAGE_BYTES) {
      return { type: 'image', name, mime: MIME[ext] || 'image/png', tooLarge: true }
    }
    return { type: 'image', name, mime: MIME[ext] || 'image/png', base64: bytesToBase64(bytes) }
  }
  if (info.size !== undefined && info.size > MAX_TEXT_BYTES) {
    return { type: 'text', name, tooLarge: true }
  }
