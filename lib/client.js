		...
		function FileTreeSidebar(props) {
			...
			react.useEffect(() => {
				...
				props.connection.rpc.call(CHANNEL, "list", { path: cwd }).then((res) => {
					if (cancelled) return;
					if (res && res.ok && res.value) { setTree(res.value.tree || null); setStatus("ok"); }
					else if (res && !res.ok) { setError((res.error && (res.error.message || res.error.code)) || "list failed"); setStatus("error"); }
					else { setError("list failed"); setStatus("error"); }
				}).catch((e) => {
					if (cancelled) return;
