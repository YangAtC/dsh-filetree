window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Stable content signature of a tree (type + path of every node). Used to
		// detect "did the tree actually change?" so a periodic refresh only settles
		// a new tree when files/dirs were added/removed/renamed, and otherwise
		// leaves the already-expanded folders untouched (preserving open state).
		function treeSig(node) {
			if (!node) return "";
			const out = [];
			(function walk(n) {
				if (!n) return;
				out.push((n.type === "directory" ? "d:" : "f:") + n.path);
				if (n.children) { for (let i = 0; i < n.children.length; i++) walk(n.children[i]); }
			})(node);
			return out.join("|");
		}
...
`;
		...
		function openFilePreview(path, connection) {
			...
			connection.rpc.call(CHANNEL, "read", { path }).then((res) => {
				...
				if (!t) return;
				if (res && res.ok && res.value) { t.loading = false; t.data = res.value; }
				else if (res && !res.ok) { t.loading = false; t.error = (res.error && (res.error.message || res.error.code)) || "read failed"; }
				else { t.loading = false; t.error = "read failed"; }
				notifyFile();
			}).catch((e) => {
				const t = getFileTab(id);
				if (!t) return;
		...
		function FileTreeSidebar(props) {
			...
			react.useEffect(() => {
				if (props.open) { try { props.open(); } catch (e) {} }
			}, []);
			react.useEffect(() => {
				...
				let pending = false;
				const refresh = () => {
					if (pending) return; // skip overlapping calls
					pending = true;
					props.connection.rpc.call(CHANNEL, "list", { path: cwd }).then((res) => {
						if (cancelled) return;
						if (res && res.ok && res.value) {
							const t = res.value.tree || null;
							const sig = t ? treeSig(t) : "";
							// Update only when the tree actually changed (so the
							// already-opened folders don't collapse on every tick).
							if (sig !== lastSig) {
								lastSig = sig;
								setTree(t);
								setStatus("ok");
							...
							setError((res.error && (res.error.message || res.error.code)) || "list failed");
							setStatus("error");
						}
					}).catch((e) => {
						if (cancelled) return;
		...
		function apply(ctx) {
			...
			// by ui-conversation's tool-details panel at priority 0; the renderer
			// picks the LOWEST-priority entry, so register at -1 to shadow it.
			ctx.effect(() => ctx.slots.inject("details", () => ctx.slots.register({
				name: "details",
				id: "dsh-filetree-sidebar",
				priority: -1,
				inject: () => ({ connection: connection, open: () => { const l = ctx.get("layout"); if (l) { try { l.openDetails(); } catch (e) {} } } })
